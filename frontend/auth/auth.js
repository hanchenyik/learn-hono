...
async function showVerification() {
  ...
  if (error || !session?.user) {
    ...
    }, '<a class="pb-button" href="/auth/resend-verification/">Resend confirmation</a><a class="pb-button alt" href="/auth/login/">Sign in</a>')
    return
  }

  if (query.get('type') === 'invite' || hash.get('type') === 'invite') {
    result({ title: 'Invitation accepted', body: 'Your email is confirmed. Choose a password to finish setting up your account.' }, '<a class="pb-button" href="/auth/reset-password/">Choose a password</a>')
    return
  }

  result({ title: 'Email confirmed', body: 'You can now sign in to PetitBakery.' }, '<a class="pb-button" href="/auth/login/">Sign in</a><a class="pb-button alt" href="/products/">Browse treats</a>')
}
...
async function init() {
  ...
  if (state === 'reset') {
    ...
    if (error || !session) {
      result({ title: 'Password link expired', body: 'Request a fresh reset email, then follow its link to choose a password.' }, '<a class="pb-button" href="/auth/forgot-password/">Request reset link</a><a class="pb-button alt" href="/auth/login/">Sign in</a>')
      return
    }
  }
  if (state === 'sent') {
    const recovery = new URLSearchParams(location.search).get('flow') === 'recovery'
    result({ title: 'Check your inbox', body: recovery ? 'Follow the password reset link to choose a new password.' : 'Follow the confirmation link to finish creating your account.' }, '<a class="pb-button" href="/auth/login/">Sign in</a><a class="pb-button alt" href="/auth/resend-verification/">Resend confirmation</a>')
    return
  }
